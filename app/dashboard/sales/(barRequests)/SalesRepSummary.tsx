...
interface SalesRepSummaryProps {
  ...
  >;
  totalExpectedForDay: number;
}

export function SalesRepSummary({
  salesRepSummary,
  totalExpectedForDay,
}: SalesRepSummaryProps) {
    ...
    <div className="space-y-4">
      ...
      <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg text-center">
        <h2 className="text-lg font-bold text-blue-700 dark:text-blue-300">
          Total Expected Sales for the Day: ₦
          {totalExpectedForDay.toLocaleString()}
        </h2>
      </div>

      {/* 💳 Individual Sales Reps Summary */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {Object.entries(salesRepSummary).map(([rep, summary]) => {
          ...
          const expectedAmount =
            summary.totalAmount + totalNonCigaretteExpenses;
